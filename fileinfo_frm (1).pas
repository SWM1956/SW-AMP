...
uses
  lazdynamic_bass, LCLIntf, Math, base64, swamp_strings;
const
  ...
  BASS_CTYPE_STREAM_MP4  = $10007;
  BASS_CTYPE_STREAM_WMA  = $10300;
  BASS_CTYPE_STREAM_APE  = $10006;
  BASS_CTYPE_STREAM_MPC  = $10A00;
  BASS_CTYPE_STREAM_WV    = $10009;
...
begin
  ...
  Title   := Trim(Copy(Title,   1, Pos(#0, Title   + #0) - 1));
  Artist  := Trim(Copy(Artist,  1, Pos(#0, Artist  + #0) - 1));
  Album   := Trim(Copy(Album,   1, Pos(#0, Album   + #0) - 1));
  Year    := Trim(Copy(Year,    1, Pos(#0, Year    + #0) - 1));
  Comment := Trim(Copy(Comment, 1, Pos(#0, Comment + #0) - 1));
...
begin
  ...
  begin
    ...
    // Коментар COMM: encoding(1) + lang(3) + short desc + #0 + text
    else if string(FrameID) = 'COMM' then
    begin
      if FrameSize > 4 then
      begin
        Encoding  := FrameData[0];
        // Пропускаємо мову (3 байти) і короткий опис до #0
        DescStart := FrameData + 4;
        DescEnd   := DescStart;
        // Знаходимо кінець short description
        while (DescEnd < FrameData + FrameSize) and (DescEnd^ <> 0) do
          Inc(DescEnd);
        Inc(DescEnd); // пропускаємо #0
...
const
  PAD   = 8;
  LW    = 95;   // ширина лейблів-підписів
  EH    = 24;   // висота TEdit
  COL2  = LW + PAD * 2;

  procedure Row(Parent: TWinControl; const Lbl: string;
    var Ed: TEdit; Y: Integer);
  var L: TLabel;
  begin
  ...
  procedure InfoRow(Parent: TWinControl; const Lbl: string;
    var Val: TLabel; Y: Integer);
  var L: TLabel;
  begin
    L := TLabel.Create(Self);
    L.Parent  := Parent;
    ...
    Val.Left    := COL2;
    Val.Top     := Y + 3;
    Val.Width   := Parent.ClientWidth - COL2 - PAD;
...
begin
  ...
  // --- Теги ---
  Y := PAD;
  Row(TabTags, rsTitle,       EdTitle,   Y); Inc(Y, EH + PAD);
  Row(TabTags, rsArtist,  EdArtist,  Y); Inc(Y, EH + PAD);
  Row(TabTags, rsAlbum,      EdAlbum,   Y); Inc(Y, EH + PAD);
  Row(TabTags, rsYear,         EdYear,    Y); Inc(Y, EH + PAD);
  Row(TabTags, rsGenre,        EdGenre,   Y); Inc(Y, EH + PAD);
  Row(TabTags, rsTrackNo,      EdTrack,   Y); Inc(Y, EH + PAD);
  Row(TabTags, rsComment,    EdComment, Y);

  // --- Аудіо ---
  Y := PAD;
  InfoRow(TabTech, rsFormat,      LblFormatVal,     Y); Inc(Y, EH + PAD);
  InfoRow(TabTech, rsBitrate,     LblBitrateVal,    Y); Inc(Y, EH + PAD);
  InfoRow(TabTech, rsSampleRate,     LblSampleRateVal,  Y); Inc(Y, EH + PAD);
  InfoRow(TabTech, rsChannels,      LblChannelsVal,   Y); Inc(Y, EH + PAD);
  InfoRow(TabTech, rsDuration,  LblDurationVal,   Y);

  // --- Файл ---
  Y := PAD;
  InfoRow(TabFile, rsFileName, LblFileNameVal, Y);
  LblFileNameVal.AutoSize := False;
  LblFileNameVal.WordWrap := True;
  LblFileNameVal.Height   := 40;
  Inc(Y, 48);
  InfoRow(TabFile, rsFileSize,  LblFileSizeVal, Y); Inc(Y, EH + PAD);
  InfoRow(TabFile, rsModified, LblModifiedVal, Y);
