...
type
 BPMBEATPROC = procedure(handle: DWORD; beatpos: DOUBLE; user: Pointer); {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};



 var BASS_FX_GetVersion:function(): DWORD; {$IFDEF windows}stdcall{$ELSE}cdecl{$ENDIF};
 (* Tempo, Pitch scaling and Sample rate changers *)
 var BASS_FX_TempoCreate:function(chan, flags: DWORD): HSTREAM; {$IFDEF windows}stdcall{$ELSE}cdecl{$ENDIF};
 var BASS_FX_TempoGetSource:function(chan: HSTREAM): DWORD; {$IFDEF windows}stdcall{$ELSE}cdecl{$ENDIF};
 var BASS_FX_TempoGetRateRatio:function(chan: HSTREAM): FLOAT; {$IFDEF windows}stdcall{$ELSE}cdecl{$ENDIF};
 (* Reverse playback *)
 BASS_FX_ReverseCreate:function(chan: Integer; dec_block: FLOAT; flags: DWORD): Integer; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
 BASS_FX_ReverseGetSource:function(chan: Integer): HSTREAM; {$IFDEF MSWINDOWS}stdcall{$ELSE}cdecl{$ENDIF};
